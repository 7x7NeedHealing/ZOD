...
const addFavorite = (index: blogAndUserInfo) => {
    ...
    axios.get(state.http + '/deleteFavorite?blogId=' + `${index.blogId}` + '&account=' + `${state.account}`)
        .then((res) => {
    ...
    setTimeout(() => {
        axios.post(state.http + '/addFavorite', { blogId: index.blogId, account: state.account })
            .then((res) => {
                console.log(res);
